...
Visualizer::Visualizer(QWidget* parent)
    ...
    // register selection
    // http://www.pcl-users.org/Select-set-of-points-using-mouse-td3424113.html

    viewer->registerKeyboardCallback(
        boost::bind(&Visualizer::KeyboardEventProcess, this, _1));
    viewer->registerAreaPickingCallback(
        boost::bind(&Visualizer::AreaPickingEventProcess, this, _1));
    viewer->registerMouseCallback(
        boost::bind(&Visualizer::MouseEventProcess, this, _1));
    ...
    connect(ui->action_detect_plane, &QAction::triggered,
            ui->PlaneDetect_dockWidget, &QDockWidget::show);
...
void Visualizer::initialize() {
    ...
    if (true/* using config */ && ifstream("./config/config.json")){
        ...
        for (auto& item : pt.get_child("labels")) {
            std::cout << item.second.data() << " ";
            Annotation::getTypes()->push_back(item.second.data());
        }
        std::cout << std::endl;
    } else{
    ...
    //    axesWidget->SetInteractor(
    //    viewer->getInteractorStyle()->GetInteractor() );
...
void Visualizer::highlightPoint(std::vector<int>& slice) {
    ...
    for (std::vector<int>::iterator it = slice.begin(); it != slice.end();
         it++) {
...
void Visualizer::typeButtonClickedProcess(string type) {
    ...
    // QString::fromStdString("information"),QString::fromStdString(type));
    if (currPickedAnnotation) {
...
void Visualizer::AreaPickingEventProcess(
    ...
    if (s && a) {  // intersection
        last_selected_slice =
            intersectionVector(last_selected_slice, new_selected_slice);
    } else if (s) {  // union
        last_selected_slice =
            unionVector(last_selected_slice, new_selected_slice);
    } else if (a) {  // remove
        last_selected_slice =
            diffVector(last_selected_slice, new_selected_slice);
    } else {  // new
...
void Visualizer::MouseEventProcess(
...
void Visualizer::planeDetect() {
    ...
    double distanceThreshold =
        ui->distanceThreshold_lineEdit->text().toDouble();
    ...
    //    pcl::ExtractIndices<PointT> eifilter (true); // Initializing with true
    //    will allow us to extract the removed indices eifilter.setInputCloud
    //    (cloud); eifilter.setIndices (inliers); eifilter.filter
    //    (*groundPlaneCloud);
    ...
    //        pcl::visualization::PointCloudColorHandlerCustom<PointT>
    //        colorHandler1(remainCloud,
    //        colors.at(1).r,colors.at(1).g,colors.at(1).b);
    ...
    //        colors.at(1).r,colors.at(1).g,colors.at(1).b);
    //        pcl::visualization::PointCloudColorHandlerCustom<PointT>
    //        colorHandler2(groundPlaneCloud,
    //        colors.at(2).r,colors.at(2).g,colors.at(2).b);
    //        viewer->addPointCloud<PointT>(remainCloud,colorHandler1,"remainCloud");
...
void Visualizer::save() { annoManager->saveAnnotations(annotationFileName); }
