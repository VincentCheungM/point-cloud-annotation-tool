...
void Annotation::picked(vtkRenderWindowInteractor *interactor) {
    ...
    // boxWidget->SetPlaceFactor( 1.25 ); // Make the box 1.25x larger than the
    // actor boxWidget->PlaceWidget();
...
void Annotaions::loadAnnotations(std::string filename) {
    ...
    std::cout << filename << ": load " << annotations.size() << " boxes"
              << std::endl;
    input.close();
}
...
std::vector<Annotation *> &Annotaions::getAnnotations() { return annotations; }
